...
fun GetDataFromOperation [] = 0
| GetDataFromOperation [{node=node, component=component, operation=operation, 
        priority=priority, deadline=deadline, enqueue_time=enqueue_time, data=data, steps=steps}] = data;

fun NetworkLogOnThisNode timestamp {node=cmq_node, cmql=cmql} [] = []
  | NetworkLogOnThisNode timestamp {node=cmq_node, cmql=cmql}
      ({node=component_node, component=component, priority=priority, operation=operation}::other_threads) = 
    if (operation <> []) then 
        []
    else
        if (IsComponentCMQEmpty component cmql) then 
            [{timestamp=timestamp, node=component_node, component=component, data=(GetDataFromOperation [(GetNextComponentOperation component cmql)])}]
        else
            (NetworkLogOnThisNode timestamp {node=cmq_node, cmql=cmql} other_threads);   

fun NetworkLog clocks ncmql [] rths = []
  | NetworkLog clocks ncmql ({node=node, threads=threads}::other_nodes) rths = 
    if ((CanScheduleOnThisNode clocks (hd (GetCMQOnNode node ncmql)) threads (FindRunningThreadOnNode node rths)) = true) then
      (NetworkLogOnThisNode (GetClockValue (hd (GetClockOnNode node clocks) )) (hd (GetCMQOnNode node ncmql)) threads)
      ^^(NetworkLog clocks ncmql other_nodes rths)
    else 
      (NetworkLog clocks ncmql other_nodes rths); 
...
fun ExecuteGuard clocks timers rths wi ni wths = 
  if ((LimitReached clocks = false) andalso
...
(*-----------------------------------------------------------------------------------------------------------------------------------------------------------*) 
(*-----------------------------------------------------------------------------------------------------------------------------------------------------------*) 
(* GET ALL NETWORK PACKETS WAITING ON A GIVEN NODE *)
(*-----------------------------------------------------------------------------------------------------------------------------------------------------------*) 
(*-----------------------------------------------------------------------------------------------------------------------------------------------------------*) 
fun GetNetworkDataOnNode this_node [] = []
  | GetNetworkDataOnNode this_node ({node=node, dequeue_time=dequeue_time, queue=queue}::other_nodes) = 
      if (node=this_node andalso queue != []) then 
        {node=node, dequeue_time=dequeue_time, queue=queue}::(GetNetworkDataOnNode this_node other_nodes)
      else
        (GetNetworkDataOnNode this_node other_nodes); 

fun NextNetworkDequeue [] np = ~1
  | NextNetworkDequeue [{node=node, dequeue_time=dequeue_time, queue=queue}] np = 

       (QueueOperationEnqueueTime (hd queue)) + 
...
fun UpdateOperationSteps {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=operation, priority=priority, deadline=deadline, 
        enqueue_time=enqueue_time, data=data, steps=[]}]} = 
  ...
  | UpdateOperationSteps {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=operation, priority=priority, deadline=deadline, 
        enqueue_time=enqueue_time, data=data, steps=({kind=kind, port=port, unblk=unblk, exec_time=exec_time, 
        duration=duration, max_data_size=max_data_size}::other_steps)}]} = 
        if (exec_time = duration) then 
            if (other_steps = []) then
                ...
                enqueue_time=enqueue_time, data=data, steps=other_steps}]}
        else
            {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=operation, priority=priority, deadline=deadline, 
        enqueue_time=enqueue_time, data=data, steps=({kind=kind, port=port, unblk=unblk, exec_time=exec_time, 
        duration=duration, max_data_size=max_data_size}::other_steps)}]};
...
fun HandleCode {node=clock_node, value=value, next_tick=next_tick} timers waiting_interactions waiting_threads ni np
  {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=operation, priority=priority, deadline=deadline, 
        enqueue_time=enqueue_time, data=data, steps=({kind=kind, port=port, unblk=unblk, exec_time=exec_time, duration=duration, max_data_size=max_data_size}
          ::other_steps)}]} = 
    if ((duration -  exec_time) > (next_tick - value)) then
      ...
      (UpdateOperationSteps
        {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=operation, priority=priority, deadline=deadline, 
          enqueue_time=enqueue_time, data=data, steps=({kind=kind, port=port, unblk=unblk, 
              exec_time=(exec_time + 
                  ((FindNextClockValue ([(NextTimerOffset (GetTimersOnNode node timers))]^^[(NextInteractionEnqueueTime (GetInteractionsOnNode node waiting_interactions))]
                                    ^^[(NextNetworkDequeue (GetNetworkDataOnNode node ni) np)]
                                    ^^[(NextThreadEnqueueTime (GetWaitingThreadsOnNode node waiting_threads))]^^[next_tick]))
                    - value)), 
              duration=duration, max_data_size=max_data_size}
          ::other_steps)}]})
    else
...
fun ExecuteOperation clocks timers waiting_interactions waiting_threads ni np
  {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=operation, priority=priority, deadline=deadline, 
      enqueue_time=enqueue_time, data=data, steps=({kind=kind, port=port, unblk=unblk, exec_time=exec_time, duration=duration, max_data_size=max_data_size}
        ::other_steps)}]} = 
  if (kind = "LOCAL") then 
    [(HandleCode 
      ...
      ni (* List o all network queues *)
      np (* List of all network profiles *)
      {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=operation, priority=priority, deadline=deadline, 
        enqueue_time=enqueue_time, data=data, steps=({kind=kind, port=port, unblk=unblk, exec_time=exec_time, duration=duration, max_data_size=max_data_size}
          ::other_steps)}]})]
  else
...
fun ExecuteThreads clocks timers waiting_interactions waiting_threads ni np [] = []
  | ExecuteThreads clocks timers waiting_interactions waiting_threads ni np ({node=node, component=component, priority=priority, operation=operation}::other_threads) = 
      (ExecuteOperation clocks timers waiting_interactions waiting_threads ni np {node=node, component=component, priority=priority, operation=operation})
      ^^(ExecuteThreads clocks timers waiting_interactions waiting_threads ni np other_threads); 
...
fun EnqueueThread {node=thread_node, component=component, priority=priority, operation=operation} [] = 
  ...
  | EnqueueThread {node=thread_node, component=component, priority=priority, operation=operation}
        ...
        if (priority > priority2) then 
          ...
          (EnqueueThread {node=thread_node, component=component, priority=priority, operation=operation} other_threads);
...
fun GetAllInducedOperations {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=thread_operation, priority=priority, deadline=deadline, 
  ...
  | GetAllInducedOperations {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=thread_operation, priority=priority, deadline=deadline, 
                           ...
                           ({node=interaction_node, port=interaction_port, operation={node=operation_node, component=operation_component, operation=operation, priority=operation_priority, 
                              deadline=operation_deadline, enqueue_time=operation_enqueue_time, data=operation_data, steps=operation_steps}}::other_interactions) 
            ...
            if (thread_node=interaction_node andalso port=interaction_port andalso (List.exists (fn z => z = kind) ["CLIENT", "PUBLISHER"])) then
                ({data=(UpdateDataSize
                  ...
                  deadline=operation_deadline, enqueue_time=value, data=operation_data, steps=operation_steps}), 
                operation=(UpdateNetworkData2 
                          {node=clock_node, value=value, next_tick=next_tick} 
                          {node=operation_node, component=operation_component, operation=operation, priority=operation_priority, 
                  deadline=operation_deadline, enqueue_time=value, data=operation_data, steps=operation_steps})}::
...
fun InduceOperations [] rths interactions clocks np = []
  | InduceOperations ({node=network_node, dequeue_time=dequeue_time, queue=queue}::other_network_queues) rths [] clocks np = ({node=network_node, dequeue_time=dequeue_time, queue=queue}::other_network_queues)
  | InduceOperations ({node=network_node, dequeue_time=dequeue_time, queue=queue}::other_network_queues) [] interactions clocks np = ({node=network_node, dequeue_time=dequeue_time, queue=queue}::other_network_queues)
  | InduceOperations ({node=network_node, dequeue_time=dequeue_time, queue=queue}::other_network_queues) 
                      ({node=thread_node, component=component, priority=priority, operation=operation}::other_threads)
                        ({node=interaction_node, port=interaction_port, operation=induced_operation}::other_interactions)
                        clocks np = 
      if (network_node = thread_node) then
        if (queue != []) then
        {node=network_node, dequeue_time = dequeue_time, (* + (GetClockValue (hd (GetClockOnNode thread_node clocks))), *)
        queue=queue^^
               (GetAllInducedOperations 
                      {node=thread_node, component=component, priority=priority, operation=operation} (* Currently Running Thread *)
                ...
                )}::(InduceOperations other_network_queues ({node=thread_node, component=component, priority=priority, operation=operation}::other_threads)
                        ({node=interaction_node, port=interaction_port, operation=induced_operation}::other_interactions)
                        clocks np)
        else
        {node=network_node, dequeue_time = dequeue_time, (* (GetClockValue (hd (GetClockOnNode thread_node clocks))), *)
        ...
        {node=network_node, dequeue_time=dequeue_time, queue=queue}
        ::(InduceOperations other_network_queues ({node=thread_node, component=component, priority=priority, operation=operation}::other_threads)
                ({node=interaction_node, port=interaction_port, operation=induced_operation}::other_interactions) clocks np);
...
fun FindMinimumAdvanceTime [] timers waiting_interactions waiting_threads ncmql nths rths ni np = ~1
  | FindMinimumAdvanceTime ({node=node, value=value, next_tick=next_tick}::other_clocks) timers waiting_interactions waiting_threads ncmql nths rths ni np = 
    (FindMinimum
        (DeleteNumber ~1 ([(NextTimerOffset (GetTimersOnNode node timers))]^^[(NextInteractionEnqueueTime (GetInteractionsOnNode node waiting_interactions))]
                            ...
                            ^^[(FindMinimumAdvanceTime other_clocks timers waiting_interactions waiting_threads ncmql nths rths ni np)])
        )
      );

fun FindMinimumAdvanceTimeWithNextTick [] timers waiting_interactions waiting_threads ncmql nths rths ni np = ~1
  | FindMinimumAdvanceTimeWithNextTick ({node=node, value=value, next_tick=next_tick}::other_clocks) timers waiting_interactions waiting_threads ncmql nths rths ni np = 
    (FindMinimum
        (DeleteNumber ~1 ([(NextTimerOffset (GetTimersOnNode node timers))]^^[(NextInteractionEnqueueTime (GetInteractionsOnNode node waiting_interactions))]
                            ^^[(NextNetworkDequeue (GetNetworkDataOnNode node ni) np)]
                            ^^[(NextThreadEnqueueTime (GetWaitingThreadsOnNode node waiting_threads))]
                            ^^[next_tick]
                            ^^[(FindMinimumAdvanceTimeWithNextTick other_clocks timers waiting_interactions waiting_threads ncmql nths rths ni np)])
        )
      );    

fun GetAdvanceTime clocks timers waiting_interactions waiting_threads ncmql nths rths ni np = 
  ...
  if ((ScheduleGuard (GotoNextTickAllClocks clocks) ncmql nths rths) = false) then 
    (FindMinimumAdvanceTime clocks timers waiting_interactions waiting_threads ncmql nths rths ni np)
  else
    (FindMinimumAdvanceTimeWithNextTick clocks timers waiting_interactions waiting_threads ncmql nths rths ni np);
...
fun AdvanceTime by_this_much [] timers waiting_interactions waiting_threads = []
  | AdvanceTime by_this_much (first_clock::other_clocks) timers waiting_interactions waiting_threads =   
        (AdvanceThisClock by_this_much first_clock)
...
fun SaveOperation clocks timers waiting_interactions network_queues np waiting_threads
  {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=operation, priority=priority, deadline=deadline, 
      enqueue_time=enqueue_time, data=data, steps=({kind=kind, port=port, unblk=unblk, exec_time=exec_time, duration=duration, max_data_size=max_data_size}
        ::other_steps)}]} = 
  if (kind = "PUBLISHER" andalso other_steps = []) then
    ...
    if (kind = "LOCAL" andalso other_steps = []) then 
      if ((ExecuteOperation clocks timers waiting_interactions waiting_threads network_queues np
              {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=operation, priority=priority, deadline=deadline, 
                enqueue_time=enqueue_time, data=data, steps=({kind=kind, port=port, unblk=unblk, exec_time=exec_time, duration=duration, max_data_size=max_data_size}
              ::other_steps)}]})
...
fun GetCompletedOperations clocks timers waiting_interactions network_queues np waiting_threads [] = []
  | GetCompletedOperations clocks timers waiting_interactions network_queues np waiting_threads ({node=node, component=component, priority=priority, operation=operation}::other_threads) = 
      (SaveOperation clocks timers waiting_interactions network_queues np waiting_threads {node=node, component=component, priority=priority, operation=operation})
      ^^(GetCompletedOperations clocks timers waiting_interactions network_queues np waiting_threads other_threads);

fun SaveAllCompletedOperations copns clocks timers waiting_interactions network_queues np waiting_threads rths = 
    copns^^(GetCompletedOperations clocks timers waiting_interactions network_queues np waiting_threads rths);
...
fun BlockThisThread clocks timers waiting_interactions network_queues np waiting_threads
  {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=operation, priority=priority, deadline=deadline, 
      enqueue_time=enqueue_time, data=data, steps=({kind=kind, port=port, unblk=unblk, exec_time=exec_time, duration=duration, max_data_size=max_data_size}
        ::other_steps)}]} = 
  if (kind = "CLIENT") then 
...
fun GetBlockedThreads clocks timers waiting_interactions network_queues np waiting_threads [] = []
  | GetBlockedThreads clocks timers waiting_interactions network_queues np waiting_threads ({node=node, component=component, priority=priority, operation=operation}::other_threads) = 
      (BlockThisThread clocks timers waiting_interactions network_queues np waiting_threads {node=node, component=component, priority=priority, operation=operation})
      ^^(GetBlockedThreads clocks timers waiting_interactions network_queues np waiting_threads other_threads); 

fun GetAllBlockedThreads bths clocks timers waiting_interactions waiting_threads network_queues np rths = 
  bths^^(GetBlockedThreads clocks timers waiting_interactions waiting_threads network_queues np rths);
...
fun GetUnblockedThreads {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=thread_operation, priority=priority, deadline=deadline, 
                        ...
                        timers wi network_queues np wths = []
  ...
  | GetUnblockedThreads {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=thread_operation, priority=priority, deadline=deadline, 
                        ...
                        ({node=blocked_node, component=blocked_component, priority=blocked_priority, 
                          operation=
                              [{node=operation_node, component=operation_component, operation=operation, priority=operation_priority, 
                              deadline=operation_deadline, enqueue_time=operation_enqueue_time, data=operation_data,
                              steps=
                                  ({kind=operation_kind, port=operation_port, unblk=operation_unblk, exec_time=operation_exec_time, duration=operation_duration, max_data_size=operation_max_data_size}
            ...
            if ((operation_kind = "CLIENT") 
                ...
                andalso              
                  ...
                  (ExecuteOperation [{node=clock_node, value=value, next_tick=next_tick}] timers wi wths network_queues np
                        {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=thread_operation, priority=priority, deadline=deadline, 
                            enqueue_time=enqueue_time, data=data, steps=({kind=kind, port=port, unblk=unblk, exec_time=exec_time, duration=duration, max_data_size=max_data_size}
                            ::other_steps)}]}) 
                      = [{node=thread_node, component=thread_component, priority=thread_priority, operation=[]}] 
                      ...
                      [{node=blocked_node, unblock_time=(GetClockValue 
                                                          (hd (GetClockOnNode thread_node 
                                                            (UpdateClocks all_clocks timers wi
                                                              ...
                                                              operation=[{node=node, component=component, operation=thread_operation, priority=priority, deadline=deadline, enqueue_time=enqueue_time, data=data, 
                                                              steps=({kind=kind, port=port, unblk=unblk, exec_time=exec_time, duration=duration, max_data_size=max_data_size}
                                                            ::other_steps)}]}])
                                                          ))
                                                        ), 
                        thread={node=blocked_node, component=blocked_component, priority=blocked_priority, 
                          operation=
                              [{node=operation_node, component=operation_component, operation=operation, priority=operation_priority, 
                              deadline=operation_deadline, enqueue_time=operation_enqueue_time, data=operation_data,
                              steps=({kind=operation_kind, port=operation_port, unblk=operation_unblk, exec_time=operation_exec_time, duration=operation_duration, max_data_size=operation_max_data_size}
                                    ::operation_other_steps)}]}}]
            else
              (GetUnblockedThreads 
                {node=thread_node, component=thread_component, priority=thread_priority, operation=[{node=node, component=component, operation=thread_operation, priority=priority, deadline=deadline, 
                            ...
                            ::other_steps)}]}
                other_blocked_threads
                {node=clock_node, value=value, next_tick=next_tick} all_clocks timers wi network_queues np wths);

fun UnblockThreads wths rths [] clocks timers wi network_queues np = []
  | UnblockThreads wths [] bths clocks timers wi network_queues np = []
  | UnblockThreads wths ({node=thread_node, component=component, priority=priority, operation=operation}::other_threads) 
                        ...
                        clocks timers wi network_queues np = 
