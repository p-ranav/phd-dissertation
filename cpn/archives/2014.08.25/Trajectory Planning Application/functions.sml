...
(* No threads in partition?! *)
fun no_ths_in_part {clock_node=clock_node, clock_value=clock_value, schedule=({part_name=part_name, exec_t=exec_t, dur=dur, pr=pr, off=off}::other_pns)}
						{nid=nid, can_sched=can_sched, node_ths=[]} = false
  | no_ths_in_part {clock_node=clock_node, clock_value=clock_value, schedule=[]}
  						{nid=nid, can_sched=can_sched, node_ths=[]} = false
  | no_ths_in_part {clock_node=clock_node, clock_value=clock_value, schedule = []}
  					  {nid=nid, can_sched=can_sched, node_ths={part=part, ths=ths}::other_part_ths} =
  				if ((part="SYSTEM") andalso (ths = [])) then true
  				else (no_ths_in_part {clock_node=clock_node, clock_value=clock_value, schedule=[]} {nid=nid, can_sched=can_sched, node_ths=other_part_ths})
  | no_ths_in_part {clock_node=clock_node, clock_value=clock_value, schedule=({part_name=part_name, exec_t=exec_t, dur=dur, pr=pr, off=off}::other_pns)}
						{nid=nid, can_sched=can_sched, node_ths={part=part, ths=ths}::other_part_ths} = 
  				if ((part_name = part) andalso (part <> "SYSTEM") andalso (ths = [])) then true
  				else 
  					if ((part = "SYSTEM") andalso (ths <> [])) then false
  					else (no_ths_in_part {clock_node=clock_node, clock_value=clock_value, schedule=({part_name=part_name, exec_t=exec_t, dur=dur, pr=pr, off=off}::other_pns)}
  										{nid=nid, can_sched=can_sched, node_ths=other_part_ths});
...
fun schedule_guard node_clock node_cmq node_threads = 
	if  (   (node_check node_clock node_cmq node_threads = true) andalso 
	         ...
	         (no_ths_in_part node_clock node_threads = false) andalso 
...
|   canInduce       {comp_node=comp_node, comp_name=comp_name, comp_part=comp_part, comp_prio=comp_prio, comp_st=comp_st, comp_op=[{opname=opname, op_node=op_node, op_comp=op_comp, op_pn=op_pn, op_prio=op_prio, op_dl=op_dl, op_st=op_st, op_et=op_et, op_exec_t=op_exec_t, 	
					op_dv=op_dv, op_calls=[]}]}
					{node_name=this_node, port_name=this_port, opn=opn} = false
|   canInduce       {comp_node=comp_node, comp_name=comp_name, comp_part=comp_part, comp_prio=comp_prio, comp_st=comp_st, comp_op=[{opname=opname, op_node=op_node, op_comp=op_comp, op_pn=op_pn, op_prio=op_prio, op_dl=op_dl, op_st=op_st, op_et=op_et, op_exec_t=op_exec_t, 
					op_dv=op_dv, op_calls=({port_type=port_type, port_name=port_name1, call_op_name=call_op_name, unblk_list=unblk_list, blk_on_complete=blk_on_complete, induction=induction, q_t=q_t, pr_t=pr_t, call_st=call_st, call_et=call_et, call_exec_t=call_exec_t, call_dur=call_dur}::other_calls)}]}
                    {node_name=this_node, port_name=this_port, opn=opn} = 

         (* In case of RMI or AMI *)
         if((comp_node = this_node) andalso (port_name1=this_port) andalso (port_type = "Receptacle" orelse port_type = "Facet") andalso (call_exec_t = q_t) andalso (induction = false))	then
         			true 
         else
...
fun update_exec_t {comp_node=comp_node2, comp_name=comp_name2, comp_part=comp_part2, comp_prio=comp_prio2, comp_st=comp_st2, comp_op=[{opname=opname2, op_node=op_node2, op_comp=op_comp2, op_pn=op_pn2, op_prio=op_prio2, op_dl=op_dl2, 
 				op_st=op_st2, op_et=op_et2, op_exec_t=op_exec_t2, op_dv=op_dv2,   op_calls=({port_type=port_type, port_name=port_name2, call_op_name=call_op_name2, unblk_list=unblk_list2, blk_on_complete=blk_on_complete2, induction=induction2, q_t=q_t2, pr_t=pr_t2, call_st=call_st2, call_et=call_et2, call_exec_t=call_exec_t2, call_dur=call_dur2}::other_calls2)}]}
...
fun isAMIcomplete {comp_node=comp_node2, comp_name=comp_name2, comp_part=comp_part2, comp_prio=comp_prio2, comp_st=comp_st2, comp_op=[]}
				...
				{opname=opname, op_node=op_node, op_comp=op_comp, op_pn=op_pn, op_prio=op_prio, op_dl=op_dl, op_st=op_st, op_et=op_et, 
	  					op_exec_t=op_exec_t, op_dv=op_dv,   op_calls=[{port_type="Receptacle", port_name=port_name, call_op_name=call_op_name, unblk_list=unblk_list, blk_on_complete=blk_on_complete, induction=induction, q_t=q_t, pr_t=pr_t, call_st=call_st, call_et=call_et, call_exec_t=call_exec_t, call_dur=call_dur}]} = false
