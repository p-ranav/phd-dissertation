...
class ROSMOD_Log_Plotter():
    ...
    def parse(self, log_contents, unit):
        ...
        for line in log_contents.split('\n'):
            if 'NODE' in line and '::COMPONENT::' in line and 'init_timer_operation' not in line:
                split_line = line.split('::')
                self.node = split_line[1]
                self.component_instance = split_line[3]
            if 'ENQUEUE' in line and 'init_timer_operation' not in line:
            ...
            if 'Execution Time' in line and 'init_timer_operation' not in line:
                if self.callbacks != []:
