...
fun FindCMQNodeAndEnqueue {node=node, component=opn_component, operation=operation, priority=priority, deadline=deadline, enqueue_time=enqueue_time, steps=steps} [] = []
  | FindCMQNodeAndEnqueue {node=node, component=opn_component, operation=operation, priority=priority, deadline=deadline, enqueue_time=enqueue_time, steps=steps}
                     ({node=cmq_node, cmql=cmql}::other_nodes) = 
      if (node = cmq_node) then
          ({node=cmq_node, cmql=(FindCMQAndEnqueue 
...
fun EnqueueAllOperations [] node_cmqs = []
  | EnqueueAllOperations (opn::other_opns) node_cmqs = 
       (FindCMQNodeAndEnqueue opn node_cmqs)^^(EnqueueAllOperations other_opns node_cmqs);
