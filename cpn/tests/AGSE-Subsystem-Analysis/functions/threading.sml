...
fun GetNonEmptyCMQ [] = []
  | GetNonEmptyCMQ ({component=component, scheme=scheme, queue=queue}::other_threads) = 
      if (queue != []) then 
          [component]::(GetNonEmptyCMQ other_threads)
      else
          (GetNonEmptyCMQ other_threads);

fun GetReadyThreadList [] = []
  | GetReadyThreadList ({node=node, component=component, priority=priority, operation=operation}::other_threads) = 
    [component]::(GetReadyThreadList other_threads);

fun ThreadCMQMatch [] readyThreads = false
  | ThreadCMQMatch nonemptyCMQs [] = false
  | ThreadCMQMatch (first::rest) readyThreads = 
      if ((List.exists (fn z => z = first) readyThreads) = true) then 
          true
      else 
          false orelse (ThreadCMQMatch rest readyThreads);
...
fun CanScheduleOnThisNode clocks {node=node, cmql=cmql} threads rths = 
	if (rths = [] andalso threads <> []) then
        ...
        (IsCMQEmpty cmql = false andalso ((ThreadCMQMatch (GetNonEmptyCMQ cmql) (GetReadyThreadList threads)) = true))) then
        (* Need to check if non-empty cmq component thread is a ready thread before returning true *)
        true
    else
        false	
	else
...
fun AdvanceTimeGuard clocks wths wi = 
  if ((LimitReached clocks = false) orelse (
